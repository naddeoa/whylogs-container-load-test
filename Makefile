n=50
SECONDS=120
REQUESTS=$(shell find ./json -type f)
...
suite:
	$(foreach request,$(REQUESTS), echo ">> $(request)"; ./hey -z $(SECONDS)s -c 4 -H 'X-API-Key: password' -m POST -D $(request) -T 'application/json' 'http://localhost:8080/logs')
